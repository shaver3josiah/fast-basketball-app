...
A private coaching tool for one basketball trainer, Coach Blake Kingsley, and the families who hire him in Fort Lauderdale, Florida. Not a social network, a marketplace or an internal business tool. It replaces schedules kept in text messages and workouts on paper, and lets a parent see what their child is told. Audience: the coach, the parents who pay him, and athletes 13 and over. Anyone can become a client through fast-basketball.com, so this is a consumer service, not an employees-only app.

HOW TO SET UP AND REACH EVERY FEATURE
Use the demo account in the fields above. It is permanently verified: no one-time code, SMS, secondary device or inbox. It is already attached to an athlete record with two conversations and a calendar. Messages is the conversation with the coach. Calendar has scheduled workouts; tapping one opens a training timer. Locker holds worksheets, including the two camera and microphone tools. You holds the consent control, notification settings, and Account, which contains deletion. Nothing is sold in the app.

EXTERNAL SERVICES
Firebase Authentication (Google) for sign-in and Cloud Firestore (Google) for messages, schedules and training records. Nothing else. No analytics, crash reporting, advertising, tracking or AI service, and no payment processor, because nothing is sold. Notifications are local only, scheduled by the device clock: there is no push server and no APNs token. One optional worksheet, Shot Form, downloads a pose-detection library from jsDelivr and its model from Google on first use; the worksheets also load fonts from Google Fonts. All of it then runs on the device.
...
Neither applies. This is youth sports coaching. All content is written by the coach or typed by the families; no licensed third-party material is included. The account holder operates the app for the coach under a signed agreement, available on request.

SAFETY, SINCE THE APP INVOLVES MINORS
A guardian is a permanent reader on every conversation between the coach and her athlete, enforced by Firestore Security Rules, not by the UI. The coach cannot remove her, nor edit or delete a message. The athlete sees a banner saying so. An athlete cannot send a message until the guardian grants consent, and she can revoke it at any moment, locking the conversation immediately. Athletes under 13 get no login; those families use the parent's account, which is why the rating is 13 and over.

USER GENERATED CONTENT, REPORTING AND BLOCKING
The only content anyone creates is a message to their own coach and answers typed into a worksheet. There is no feed, profile or search, and no way for one user to reach another, so nobody meets a stranger's content. Reporting: You tab, "Report a concern", which opens an email to the account holder. It deliberately does not go to the coach, so an athlete can report the one adult they talk to. Blocking: the guardian's consent switch on the You tab immediately stops her athlete's conversation, and each conversation can be muted. We will add any further control Apple considers necessary.

CAMERA AND MICROPHONE
Two optional training tools use them. Shot Form watches a shooting motion; the Dribble Counter listens for a bouncing ball. No image or audio is ever recorded or transmitted. The Dribble Counter does save its bounce count and a short numeric fingerprint of the ball's sound, so it recognises the same ball next time, to the athlete's own record when they tap Save. Each asks before opening the hardware, and the app works fully without either.
